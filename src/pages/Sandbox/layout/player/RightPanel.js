...
const RightPanel = () => {
    const [contentState, setContentState, timer, useTimer] = useContext(ContentStateContext); // Access the ContentState context
                        ...
                        <div className={styles.buttonWrap}>
                            ...
                            <div
                                ...
                                onClick={() => {
                                    if (!contentState.mp4ready) return;
                                    (async () => {
                                        const data = await chrome.storage.local.get(["historyLogs", "historyLogsRecordingStartedTimestamp"]);

                                        const zip = new JSZip();
                                        zip.file("gael_annotate_" + data.historyLogsRecordingStartedTimestamp + '.json', JSON.stringify(data));
                                        const blob = await zip.generateAsync({ type: "blob" }), url = window.URL.createObjectURL(blob);

                                        chrome.downloads.download(
                                            {
                                                url: url,
                                                filename: "gael_annotate_" + data.historyLogsRecordingStartedTimestamp + ".zip",
                                            },
                                            () => {
                                                window.URL.revokeObjectURL(url);
                                            }
                            ...
                            >
                                ...
                                <div className={styles.buttonMiddle}>
                                    <div className={styles.buttonTitle}>Download History Logs</div>
                                    <div className={styles.buttonDescription}>
                                        Download the history logs of this recording's annotations.
                                    </div>
                                </div>
                                <div className={styles.buttonRight}>
                                    <ReactSVG src={URL + "editor/icons/right-arrow.svg"} />
