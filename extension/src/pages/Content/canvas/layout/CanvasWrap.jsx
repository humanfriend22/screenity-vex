...
// Canvas utils
import {
    undoCanvas as originalUndoCanvas,
    redoCanvas as originalRedoCanvas,
    saveCanvas as originalSaveCanvas,
    checkChanges as originalCheckChanges,
} from "../modules/History";

const CanvasWrap = (props) => {
    const [contentState, setContentState, _timer, _setTimer, historyLogs, setHistoryLogs] = useContext(contentStateContext);
    const contentStateRef = useRef(null);
    const canvasContainer = useRef();
    const canvasRef = useRef();
    const fabricRef = useRef();

    const saveCanvas = (...args) => originalSaveCanvas(...(args.concat([[historyLogs, setHistoryLogs]])));
    const redoCanvas = (...args) => originalRedoCanvas(...(args.concat([[historyLogs, setHistoryLogs]])));
    const undoCanvas = (...args) => originalUndoCanvas(...(args.concat([[historyLogs, setHistoryLogs]])));
    const checkChanges = (...args) => originalCheckChanges(...(args.concat([[historyLogs, setHistoryLogs]])));
